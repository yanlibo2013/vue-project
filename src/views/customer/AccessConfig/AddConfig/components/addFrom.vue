<template>
  <div class="add-from">
    <el-dialog
      :title="formType == 'add' ? '新建接入配置' : '编辑接入配置'"
    ...
    >
      <el-form :model="form" class="dialog-form">
        ...
        <el-form-item label="IP白名单" :label-width="formLabelWidth">
          <el-row>
            <el-col :span="16">
              <el-row v-for="(item, index) in listFiledDefalut" :key="index">
                <el-col :span="22"
...
export default {
  ...
  methods: {
    ...
    deleteItem(val) {
      console.log(" deleteItem(val) {", val);
      this.listFiled = _.filter(_.cloneDeep(this.listFiled), (item, index) => {
